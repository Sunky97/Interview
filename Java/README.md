...
- **차이점**
    - 서로 다른 목적을 가지고 있다.
        ...
        - 인터페이스는 서로 이 없는 클래스에서 공통적으로 사용하는 방식이 필요하지만 기능을 각각 구현할 필요가 있는 경우에 사용한다.(구현 객체의 같은 동작을 보장)
...
- 특징
    ...
    2. 스트림은 재사용이 가능한 컬렉션과는 달리, 최종 연산 완료 후 Stream 이 닫히므로 단 한 번만 사용할 수 있다.
    3. 스트림은 원본 데이터를 변경하지 않는다.
    4. 스트림의 연산은 필터-맵(filter-map)기반의 API를 사용하여 지연(lazy) 연산을 통해 성능을 최적화 한다.
    5. 스트림은 parallel()메서드를 통해 손쉬운 병렬 처리를 지원한다.
- 장점
    - 불필요한 코딩 과정을 줄일 수 있고, 코드 가독성 향상
...
- 람다는 함수형 인터페이스를 통해서만 사용가능
    - 함수형 인터페이스
    - 인터페이스 내에 한 개의 추사 메소드만 정의
...
- **람다의 단점**
    - 남용하면 코드를 이해하는데 더 어려워짐
    - 까다로운 재귀 활용 및 디버깅
- 컴파일러가 Lambda 표현식을 처리하는 방법
- 익명 함수와는 다르게 컴파일
- 익명 함수는 별도 클래스의 인스턴스를 생성
- Lambda 는 'invokedynamic'을 사용하여 람다 표현식 구현을 위한 코드 생성을 런타임 시 까지 연기
...
- **동기화(Synchronized)**
    ...
    ![synchronous](https://img1.daumcdn.net/thumb/R1280x0/?scode=mtistory2&fname=https%3A%2F%2Fblog.kakaocdn.net%2Fdn%2Fcs2zHt%2FbtqF1aJFTdB%2FsP2Pzm6ZAiNnpUKzJvMJLK%2Fimg.png)
- **비동기화(Asynchronized)**
    - 비동기화 방식은 멀티 스레드 환경에서 각각의 스레드가 한 가지 자원에 대해 동시에 접근하는 방식이다.
    ![Asynchronous](https://img1.daumcdn.net/thumb/R1280x0/?scode=mtistory2&fname=https%3A%2F%2Fblog.kakaocdn.net%2Fdn%2FdkpuUa%2FbtqF2RhRoUu%2FUaSnCg4Rk0EaLj6fAjQ6H0%2Fimg.png)
    
## 리플렉션(Reflection)

#### 리플렉션 ?
> 리플렉션은 구체적인 클래스 타입을 알지 못해도, 그 클래스의 메소드, 타입, 변수들에 접근할 수 있도록 해주는 자바 API 이다.
...
- 클래스와 멤버 필드 그리고 메서드 등에 관한 정보를 얻어야 할 때
- 리플렉션 없이도 완성도 높은 코드를 구현할 수 있지만 사용한다면 조금 더 유연한 코드를 만들 수 있다.

#### 주의할 점
- 외부에 공개되지 않은 private 멤버도 `Field.setAccessible()` 메서드를 true 로 지정하면 접근과 조작이 가능하기 때문에 주의해야 한다.

## GC 란?
GC(Garbage Collector) 란 JVM 에서 Heap 영역에 남아있는, 더 이상 사용되지 않는 인스턴스들(가비지)을, 메모리가 부족할때 자동으로 삭제하여 메모리 공간을 만드는 작업을 뜻한다.

## GC 의 장점 단점
- **장점**
    - 백그라운드에서 GC 가 자동으로 메모리를 관리해주기 때문에 프로그래머가 메모리를 직접 관리할 필요가 없다.
    - 유효하지 않은 포인터에 접근하지 않는다.
- **단점**
    - 어떤 메모리를 해제할지 결정하는 데 비용이 든다.
    - GC 가 일어나는 시점이나 점유 시간을 미리 예측하기 어렵다.
